...
export default function RadialOrbitalTimeline({
  ...
  const [rotationAngle, setRotationAngle] = useState<number>(270);
      ...
      <div className="relative w-full max-w-4xl h-full flex items-center justify-center">
        ...
        >
          ...
          <div className="absolute w-28 h-28 rounded-full bg-gradient-to-br from-green-400 via-emerald-500 to-teal-500 animate-pulse flex items-center justify-center z-10">
            <div className="absolute w-36 h-36 rounded-full border border-green-400/30 animate-ping opacity-70"></div>
            <div
          ...
          {/* Orbit ring */}
          <div className="absolute rounded-full border border-green-400/20" style={{ width: '520px', height: '520px' }}></div>
          ...
          {timelineData.map((item, index) => {
            ...
            return (
              ...
              <div className="relative">
                {/* Glow halo */}
                <div
                  className={`absolute rounded-full -inset-1 transition-all duration-300 ${isPulsing ? "animate-pulse duration-1000" : ""}`}
                  style={{
                    background: isHovered
                      ? `radial-gradient(circle, rgba(74,222,128,0.5) 0%, rgba(74,222,128,0) 70%)`
                      : `radial-gradient(circle, rgba(255,255,255,0.2) 0%, rgba(255,255,255,0) 70%)`,
                    width: `${item.energy * 0.5 + 40}px`,
                    height: `${item.energy * 0.5 + 40}px`,
                    left: `-${(item.energy * 0.5 + 40 - 40) / 2}px`,
                    top: `-${(item.energy * 0.5 + 40 - 40) / 2}px`,
                    boxShadow: isHovered ? '0 0 28px 8px rgba(74,222,128,0.3)' : 'none',
                  }}
                />
                ...
                <div
                  className={`
                    ...
                    ${isExpanded
                      ? "bg-green-400 text-black border-green-300 shadow-lg shadow-green-400/40 scale-150"
                      : isHovered
                      ? "bg-green-400/25 text-green-300 border-green-400 scale-110"
                      : isRelated
                      ? "bg-green-400/15 text-green-300 border-green-400/60 animate-pulse"
                      : "bg-black/60 text-green-300/70 border-green-400/25"}
                  `}
                  style={isHovered ? { boxShadow: '0 0 18px rgba(74,222,128,0.7)' } : {}}
                >
                  <Icon size={22} />
                </div>
                ...
                <div
                  className={`
                    ...
                    ${isExpanded || isHovered ? "text-green-300 scale-110" : "text-white/80"}
                  `}
                >
                  {item.title}
                </div>
                ...
                {/* Expanded card */}
                {isExpanded && (
                  <Card className="absolute top-[6rem] left-1/2 -translate-x-1/2 w-64 bg-black/90 backdrop-blur-lg border-white/30 shadow-xl shadow-white/10 overflow-visible">
                    ...
                    <CardHeader className="pb-2">
                      ...
                      <CardTitle className="text-sm mt-2 text-white font-bold">{item.title}</CardTitle>
                    </CardHeader>
